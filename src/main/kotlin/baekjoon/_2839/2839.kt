...
fun main() {
    ...
    val rawWeight: Int = bf.readLine().toInt()
...
//        bw.write("$weightFive $weightThree ${rawWeight - weightFive * 5} ${weightFive*5+weightThree*3}\baekjoon.getN")
        if (weightFive * 5 + weightThree * 3 == rawWeight) {
            ...
            weightFive--
            weightThree = (rawWeight - weightFive * 5)/3
        }
    }
    bw.write("${if (weightFive < 0) -1 else weightFive + weightThree}\n")
//    bw.write("$weightFive $weightThree")
    bw.flush()
    bw.close()
    bf.close()
}